...
def decodage_brut(phrase):
  ...
  dico = open(
    r"mini_projet\Français.txt"
  )  #ouvre un fichier texte situé au chemin spécifié et affecte son contenu à la variable 'dico'. Ce fichier texte sera utilisé
  ...
  while decalage <= 256:  #démarre une boucle while qui va itérer 256 fois. La boucle continuera jusqu'à ce que la variable 'decalage' soit supérieure à 256.
    ...
    fragment1 = liste.split(
      " "
    )  #divise la variable 'liste' en une liste de mots en utilisant le caractère espace comme délimiteur, et affecte le résultat à la variable 'fragment1'.
    ...
    for mot in fragment1:  #démarre une boucle for qui va itérer à travers chaque mot de la liste 'fragment1'.
      if mot in read:  #vérifie si le mot courant dans la liste 'fragment1' est présent dans la variable 'read' (i.e., le contenu du fichier dictionnaire).
        print(
          "La phrase décoder est :", liste, ", la clé de chiffrement est :"
          "", decalage - 1
        )  # imprime la phrase décodée et la clé utilisée pour la déchiffrer si un mot valide est trouvé dans la phrase décodée.
